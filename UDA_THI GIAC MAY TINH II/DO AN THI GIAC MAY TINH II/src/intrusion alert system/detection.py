...
from supervision import LabelAnnotator, Detections
from supervision import BoxCornerAnnotator, Color
...
class PersonDetection:
    def __init__(self, capture_index, email_notification):
        ...
        self.alert_interval = timedelta(seconds=30) 
    ...
    def predict(self, img):
        ...
        # In Yolov8 model, objects with class_id 0 refer to a person. 
        # So, we should filter objects detected to only consider person
        detections = detections[detections.class_id == 0]

        return detections
    
    def plot_bboxes(self, detections: Detections, img):
        # labels = [f"Intruder #{track_id}" for track_id in 
        # detections.tracker_id if len(detections.tracker_id) > 0]
        ...
        try:
            while True:
                ...
                if results:
                    ...
                    # if (datetime.now() - self.last_alert_time) > self.alert_interval:                                        
                    #     self.last_alert_time = datetime.now()
